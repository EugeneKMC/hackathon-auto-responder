...
export const invoiceController = {
  async preview(c: Context) {
    try {
      ...
      const { result, error } = await invoiceService.getPreview(client_id);
  ...
  async list(c: Context) {
    try {
      ...
      const { result, error } = await invoiceService.list(client_id, direction);
      if (error) return c.json({ error }, error.statusCode as 500);
  ...
  async detail(c: Context) {
    try {
      const { client_id } = c.get('account');
      const { result, error } = await invoiceService.getById(
        client_id,
        c.req.param('id') ?? ''
      );
